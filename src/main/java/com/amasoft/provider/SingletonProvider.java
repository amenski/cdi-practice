...
public class SingletonProvider {
    ...
    public static void setInstance(SingletonProvider instance) {
        SingletonProvider.instance = instance;
    }

    private final Map<Class<?>, Class<?>> implementationRegistry = new ConcurrentHashMap<Class<?>, Class<?>>();
...
//        CdiPostProcessor<T> postProcessor = CdiPostProcessorProvider.getInstance().getPostProcessor(clz);
...
//        }
    ...
    public void register(Class<?> clazz, Class<?> implementationClass) {
        implementationRegistry.putIfAbsent(clazz, implementationClass);
    }
    ...
    private <T> Class<T> findImplementationClass(Class<T> interfaceClass) {
        ...
        String implClassName = interfaceClass.getName() + "Impl"; // FIXME Proxy.newProxyInstance(...)?
        try {
