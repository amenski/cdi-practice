    ...
    private static Field getField(Class<?> clazz, String fieldName, Class<?> original) {
        ...
        try {
            ...
            if (logger.isTraceEnabled()) {
                logger.trace("found field {} in {}", fieldName, getClassNameSafe(clazz));
            }
...
//            throw new SquirrelRuntimeException(e, ErrorCodes.CONSTRUCTOR_NOT_FOUND, type.getName(), Arrays.toString(parameterTypes));
            throw new RuntimeException("Unable to get constructor.");
        }
    }
    ...
    private static <T> T newInstance(Class<T> clazz, Constructor<T> constructor, Object[] args) {
        ...
        if (logger.isTraceEnabled()) {
            logger.trace("Creating new instance for class {} with args {}", getClassNameSafe(clazz), Arrays.toString(args));
        }
        if (constructor == null) {
...
//            throw new SquirrelRuntimeException(ErrorCodes.FIELD_NULL);
        ...
        try {
            ...
            if (logger.isTraceEnabled()) {
                logger.trace("got value {} from field {}", value, field.getName());
            }
            return value;
        } catch (Exception e) {
//            throw new SquirrelRuntimeException(e, ErrorCodes.CANNOT_GET_FIELD_VALUE, field.getName());
            throw new RuntimeException("Cant get field.");
        } finally {
...
//                    method, Arrays.toString(args), target, e.getMessage());
    ...
    public static boolean isBeanProperty(Method method) {
        ...
        if (methodName.startsWith("get") && !methodName.equals("getClass")) {    // getter method must starts with 'get' and it is not getClass()
...
//            throw new SquirrelRuntimeException(ErrorCodes.ILLEGAL_CLASS_NAME);
            throw new IllegalArgumentException("Illegal class name error");
        }
